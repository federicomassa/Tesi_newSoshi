...
int HiggsEventSelector::CheckRankedPtCut() const {
  ...
  for (auto itr = pt.begin();
    ...
    checkRanked = checkRanked && 
      (pt[pt.size() -1 - (itr - pt.begin())] > m_rankedPtCut[pt.size() -1 - (itr - pt.begin())]);
    if (!checkRanked) return returnCode[itr - pt.begin()];
  }
...
int HiggsEventSelector::CheckOnShellMassCut(const std::pair<std::vector<xAOD::TrackParticleContainer::const_iterator>,
  ...
  */
  ...
  checkCut = (isComb1Valid || isComb2Valid) ? 0 : 8;
  return checkCut;
